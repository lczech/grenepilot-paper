...
int main( int argc, char** argv )
{
    ...
    for( auto record = VcfInputIterator( flle_file ); record; ++record ) {
        ...
        for( auto sample : record->get_format_int( "AD" )) {
            ...
            data[{ record->get_chromosome(), record->get_position() }].flle_freqs[smp_cnt] = af;
            ++smp_cnt;
        }
    ...
    std::array<std::vector<double>, 16> flle_freqs;
    for( size_t i = 0; i < 16; ++i ) {
        flle_freqs[i].reserve( data.size() );
        for( auto const& entry : data ) {
            flle_freqs[i].push_back( entry.second.flle_freqs[i] );
        }
        if( flle_freqs[i].size() != data.size() ) {
            LOG_ERR << "diff size " << data.size() << " != " << flle_freqs[i].size();
        }
    }

    // Prep output
    std::ofstream csv_out( "freq_correlation.csv" );
    csv_out << "LHS\tRHS\tPCC\n";

    // Compute correlation between seeds and all flle columns.
    for( size_t i = 0; i < 16; ++i ) {
        csv_out << "seed\t" << smp_to_name[sample_names[i]] << "\t";
        csv_out << pearson_correlation_coefficient( seed_freqs, flle_freqs[i] ) << "\n";
    }
    ...
    for( size_t i = 0; i < 16; ++i ) {
        for( size_t j = i+1; j < 16; ++j ) {
            ...
            csv_out << pearson_correlation_coefficient( flle_freqs[i], flle_freqs[j] ) << "\n";
        }
    }

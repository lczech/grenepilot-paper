...
data <- read.csv("sim_results_fst_poster.csv")
...
make_plot <- function( ax, ay, ag, fact, tx, ty ) {
    ...
    pl <- ggplot(data, aes_string(x=ax, y=ay, color=colorag)) +
        geom_abline(slope=1) +
        geom_point() +
        scale_color_manual(
        ...
        coord_fixed(xlim=c(0, 0.4), ylim=c(0, 0.4), ratio = 1) +
        xlab(tx) +
        ylab(ty) +
        labs(color=tt) +
        theme(legend.position = "top") +
        theme(plot.margin=grid::unit(c(0,0,0,0), "mm"), legend.position = c(0.67, 0.15))
    ...
    # ggsave(paste0("fst_plot_svg/",ax,"-",ay,"-",ag,".svg"), bg="white", width=9, height=5.5)
...
pl_k_ps <- make_plot(
    "true_nei_fst", "est_kofler", "pool_size", TRUE,
    "True FST (Nei)", "Kofler Estimator FST"
)
pl_k_rd <- make_plot(
    "true_nei_fst", "est_kofler", "read_depth", TRUE,
    "True FST (Nei)", "Kofler Estimator FST"
)
pl_k <- plot_grid(pl_k_ps, pl_k_rd, labels = "AUTO")
ggsave("fst_plot_kofler.svg", bg="white", width=9, height=5.5)

pl_s_ps <- make_plot(
    "true_nei_fst", "est_spence_nei", "pool_size", TRUE,
    "True FST (Nei)", "Unbiased Pool-Seq Estimator FST (Nei)"
)
pl_s_rd <- make_plot(
    "true_nei_fst", "est_spence_nei", "read_depth", TRUE,
    "True FST (Nei)", "Unbiased Pool-Seq Estimator FST (Nei)"
)
pl_s <- plot_grid(pl_s_ps, pl_s_rd, labels = "AUTO")
ggsave("fst_plot_spence.svg", bg="white", width=9, height=5.5)
